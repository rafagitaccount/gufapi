...
from core.auth import authenticate, criar_token_acesso
from core.deps import get_current_user, get_session
from core.security import make_password_hash, verify_password
from models.usuario_model import UsuarioModel
from schemas.usuario_schema import (UsuarioSchemaArtigos, UsuarioSchemaBase,
                                    UsuarioSchemaCreate, UsuarioSchemaUp)
...
async def post_usuario(usuario: UsuarioSchemaCreate,
    ...
    novo_usuario: UsuarioModel = UsuarioModel(
        ...
        email=usuario.email,
        senha=make_password_hash(usuario.senha),
    ...
    async with db as session:
        try:
            ...
            raise HTTPException(detail='Essa conta já foi cadastrada!',
...
async def put_usuario(usuario_id: int,
    ...
    async with db as session:
        ...
        if usuario_up:
            ...
            if usuario.senha:
                usuario_up.senha = make_password_hash(usuario.senha)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(),
                db: AsyncSession = Depends(get_session)):
    usuario = await authenticate(email=form_data.username,
                                 password=form_data.password, db=db)
    if not usuario:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail='Credenciais de acesso incorretas.')
